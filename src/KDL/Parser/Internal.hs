...
p_document = label "Document" $ do
  bom <- option "" . withSource_ $ p_bom
  version <- option "" . withSource_ $ try p_version
  nodes <- p_nodes
  eof
...
p_base_node = label "Base Node" $ do
  slashdashNode <- p_slashdash'opt
  fmap slashdashNode $ do
    -- node ann
    ...
    (entries, entriesTrailing) <- manyWithSlashdash . try $ do
      ...
      slashdashEntry <-
        if Text.null entryLeading
          then p_slashdash
          else p_slashdash'opt
      entry <- p_node_prop_or_arg
      pure . slashdashEntry $ prependLeading entryLeading entry
...
p_decimal = label "Decimal" $ do
  signed <- parseSigned
  (i, _) <- p_integer
  (f, fdigits) <- option (0, 0) $ char '.' *> p_integer
...
p_slashdash :: Parser (a -> SlashdashResult a)
p_slashdash = label "Slashdash comment" $ do
  string "/-" *> many p_line_space *> pure (const SlashDashed)

p_slashdash'opt :: Parser (a -> SlashdashResult a)
p_slashdash'opt = option NoSlashDash p_slashdash
